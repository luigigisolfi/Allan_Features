...
def generate_label_from_csv(csv_file, template_content, mappings, VLBI = None):
    ...
    with open(csv_file, 'r') as metadata_file:
        ...
        if VLBI:
            label_content = label_content.replace('${field6_location}', row[mappings['field6_location']])
            label_content = label_content.replace('${field6_length}', row[mappings['field6_length']])
...
if __name__ == "__main__":
    ...
    print(config)
    VLBI = False
    if config['template'] == 'template_vlbi.xml':
        VLBI = True
    ...
    for root, _, files in os.walk(dataset_folder):
        for file_name in files:
            if file_name.endswith('.csv'):
                ...
                generate_label_from_csv(csv_file, template_content, mappings, VLBI)
