...
def get_baseband_frequencies_file(mission, experiment_name):
    ...
    tuple: A tuple containing:
        - baseband_frequency (dict): A dictionary mapping each station to its baseband frequency
          (in MHz), rounded to the 3rd decimal place.
    ...
    for station, channels in result.items():
        for channel in channels:
            ...
            if  float_frequency < mas_x_band < float_frequency + float_bandwidth :
                ...
                baseband_frequencies_list.append(float_frequency)

    baseband_frequency_index = np.argmin(np.abs(np.array(baseband_frequencies_list) - mas_x_band))
    baseband_frequency[mission][station] = np.round(baseband_frequencies_list[baseband_frequency_index],3)

    # Write to file
    os.makedirs(f"baseband_frequencies/{mission}/", exist_ok=True)
...
def get_baseband_frequency(mission, experiment_name, fdets_file):
            ...
            with open(frequencies_file_name, 'r') as f:
                ...
                for line in lines:
                    ...
                    if station_fdets in line:
                        baseband_frequencies_list.append(float(line.split()[1]))
    ...
    baseband_frequency_index = np.argmin(np.abs(np.array(baseband_frequencies_list) - mas_x_band))
    baseband_frequency = baseband_frequencies_list[baseband_frequency_index]
            ...
            for line in lines:
                if station in line:
                    ...
                    with open(frequencies_file_name, 'a') as a:
                        print(f'Appending missing station to file: {a}')
...
def assign_missing_baseband_frequencies(mission_name, experiment_name):
    ...
    BBC_dict = extract_station_mapping(extract_vex_block(f'./vex_files/{mission_name}/{vex_file_name}', 'BBC'), 'BBC')
    IF_dict =  extract_station_mapping(extract_vex_block(f'./vex_files/{mission_name}/{vex_file_name}', 'IF'), 'IF')

    IF_keys, IF_values = IF_dict.keys(), IF_dict.values()
    FREQ_dict =  extract_station_mapping(extract_vex_block(f'./vex_files/{mission_name}/{vex_file_name}', 'FREQ'), 'FREQ')
