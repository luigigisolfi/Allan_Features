...
from baseband_library import get_baseband_frequencies_file, get_mission_from_experiment, experiments
...
elif isinstance(args.filename, list):  # Check if it's already a list
    files = args.filename  # If it's a list, use it directly
else:
    files = [args.filename]  # Single file, put it in a list for consistency
...
for filename in files:
    mission_name = re.sub(r'\d', '', filename.split('.')[1])  # Remove all digits (if present) from mission name (ex. in mex2013, remove 2013)
    ...
    if args.experiment_name:
        ...
        if mission_name.lower() != get_mission_from_experiment(experiment_name):
            ...
            experiments_list_for_mission = [
                exp_key
                for exp_key, exp_details in experiments.items()
