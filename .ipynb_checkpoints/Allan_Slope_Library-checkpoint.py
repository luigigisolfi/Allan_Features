        ...
        def plot_parameters_error_bounds(self, extracted_data, save_dir=None, suppress=False): 
            ...
            if extracted_data != None:
                ...
                for i, dt in enumerate(utc_datetime):
                    ...
                    if hour != last_hour:
                        xticks.append(i)
                        xtick_labels.append(dt.strftime("%H:%M"))
                        last_hour = hour
                ...
                # Plot Signal-to-Noise vs UTC time
                axs[1].scatter(range(len(utc_datetime)), signal_to_noise, label=f'SNR', marker='o', linestyle='-', color='blue', s = 5)
                axs[1].set_xlabel(f'UTC Time (HH:MM:SS) on {utc_date}')
                ...
                # Plot Doppler noise [Hz] vs UTC time
                axs[2].scatter(range(len(utc_datetime)), doppler_noise_hz, label=f'Doppler Noise', marker='o', linestyle='-', color='orange', s = 5)
                axs[2].set_xlabel(f'UTC Time (HH:MM:SS) on {utc_date}')
               ...
               # Plot the Modified Allan Deviation and the white noise line
                axs[3].scatter(range(len(utc_datetime)), frequency_detection,  marker='o', color='black', label=r'Freq. Detection', s = 2)
