...
# Define the path to the parent folder
old_format_files_folder = f"/Users/lgisolfi/Desktop/data_archiving-2.0/juice/"
vex_year_month_list  = [name for name in os.listdir(old_format_files_folder) if os.path.isdir(os.path.join(old_format_files_folder, name))]

for vex_year_month in vex_year_month_list:
    if vex_year_month not in ["jc230508", "jc230914"] :
        continue
    parent_folder =  f"/Users/lgisolfi/Desktop/data_archiving-2.0/juice/{vex_year_month}"
    print(parent_folder)
    files = glob.glob(f"/Users/lgisolfi/Desktop/data_archiving-2.0/juice/{vex_year_month}/input/single/Fdets.*.r2i.txt")
    output_folder = f"/Users/lgisolfi/Desktop/data_archiving-2.0/juice/{vex_year_month}/input/complete"
    utilities.create_complete_scan_from_single_scans(files, output_folder)